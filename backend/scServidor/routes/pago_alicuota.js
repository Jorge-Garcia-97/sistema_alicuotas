...
router.post("/pagoalicuota/save/", (req, res) => {
    try {
      ...
      const query = `INSERT INTO pago_alicuota (idpago_alicuota, valor_alicuota, fecha_alicuota, estado_alicuota, atraso_alicuota) VALUES ('${data.idpago_alicuota}', '${data.valor_alicuota}', '${data.fecha_alicuota}','${data.estado_alicuota}','${data.atraso_alicuota}')`;
      getConnection(function (err, conn) {
        if (err) {
          return res.status(500).send("¡Algo ha salido mal!");
        } else {
...
router.post("/pagoalicuota/edit/estado/:id/", (req, res) => {
  try {
    ...
    const query = `UPDATE pago_alicuota SET estado_alicuota= '${data.estado_alicuota}' WHERE idpago_alicuota = ?`;
    getConnection(function (err, conn) {
      if (err) {
